...
| `snapshot-leaderboard` | Daily cron job populating `leaderboard_snapshots` (`leaderboard.html` tier-2 fallback; also the table `omega-chart.js`'s Authority History chart reads, per the fix in a prior session). ⚠️✅ Its upsert payload referenced 4 columns (`display_name`, `sign`, `tier`, `is_owner`) that didn't exist on the table — the cron has silently written 0 rows on every run since deployment, with `{ok:true}` masking the failure. Fixed this session by adding the missing columns (`supabase/omega_leaderboard_snapshots_columns_fix.sql`), verified against a real scratch PostgreSQL 16 instance. **Applied live and verified** (2026-08-17, `CLAUDE.md` §8) — confirmed via `information_schema.columns` on the live project before and after applying; the cron job's next scheduled/owner-triggered run is the first that will actually write rows. |
| `market-price` | **New this session.** Stock/ETF price lookup for `investment.html`'s "LIVE PRICE" button — that button previously covered crypto only (via CoinGecko, client-side, keyless) and told the user outright that no stock/ETF source was wired up (`GAP_ANALYSIS.md` §4.11 area / `CLAUDE.md` Part 1 discussion). Proxies Twelve Data's `/price` endpoint since a real-time equity/ETF quote API requires a key, which per this repo's own secrets convention (`CLAUDE.md` §9) can never ship client-side. Deliberately has no per-user auth check, unlike `checkout`/`stripe-webhook` — `investment.html` is one of the finance pages kept `localStorage`-only on purpose and never loads the Supabase client at all (`GAP_ANALYSIS.md` §4.2); requiring login just for a public stock quote would add a new dependency to a page kept deliberately simple. Matches `intel-feed`'s precedent instead (public-data proxy, no identity to protect, bounded by a short `Cache-Control` rather than a login wall). Double-gated on the `TWELVE_DATA_API_KEY` secret exactly like `checkout` gates on `STRIPE_SECRET_KEY` — degrades to a clear "not configured" response, not a silent failure, until the owner sets the key (`scripts/check-secrets.sh` updated to check for it). **Not yet deployed or configured live** — the secret has not been provisioned; this ships the code path dormant until the owner runs `supabase secrets set TWELVE_DATA_API_KEY=... && supabase functions deploy market-price`. |
...
- **RLS policy-correctness audit (this session):** every `FOR INSERT`/`UPDATE`/`ALL` policy
  ...
  work, owner can now read KYC uploads) — not just read by eye. **Applied live and verified**
  (2026-08-17, `CLAUDE.md` §8) — confirmed via `pg_policies` on the live project before and
  after applying.
