...
CREATE OR REPLACE FUNCTION public.authority_score(p_a numeric, p_b numeric, p_c numeric)
RETURNS numeric LANGUAGE sql IMMUTABLE AS $$
  ...
  )*1.6180339887/2.7182818285::numeric,6);
$$;
GRANT EXECUTE ON FUNCTION public.authority_score(numeric,numeric,numeric) TO authenticated,anon;

CREATE OR REPLACE FUNCTION public.lattice_node(p_track int,p_phase int,p_a numeric,p_b numeric,p_c numeric)
...
BEGIN
  ...
  RETURN QUERY SELECT
    ...
    COALESCE(p.sign,''), COALESCE(p.element,''),
    COALESCE(p.axis_a,0.001), COALESCE(p.axis_b,0.001), COALESCE(p.axis_c,0.001),
    ...
    round(public.lattice_node(p.matrix_track,p.matrix_phase,p.axis_a,p.axis_b,p.axis_c)::numeric/104976*100,4)
...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT jsonb_build_object(
    ...
    'authority_apex',27.8367,'node',
    public.lattice_node(matrix_track,matrix_phase,axis_a,axis_b,axis_c),
    'lattice_total',104976,'phases_done',COALESCE(phases_done,0),
...
INSERT INTO public.platform_settings (key,text_value) VALUES
  ('auth_phi','1.6180339887'),('auth_e','2.7182818285'),
  ('auth_apex','27.8367'),('auth_formula','sqrt(A^3+B^3+C^3)*phi/e'),
...
-- verify
SELECT public.authority_score(9,9,9) AS apex_expect_27_8367;
SELECT public.lattice_node(12,12,9,9,9) AS node_expect_104976;
