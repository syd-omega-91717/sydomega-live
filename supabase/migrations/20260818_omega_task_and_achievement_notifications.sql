-- ============================================================================
-- SYD OMEGA 91717 -- TASK COMPLETION + ACHIEVEMENT NOTIFICATIONS (Phase 3a)
-- omega_notify_triggers.sql wired member-status RPCs (approve, grant, reject,
-- revoke, extend_trial) to record notifications. This closes the gap for the
-- two achievement-progression events that occur on every approved member's visit:
--   1. Task completion (complete_task() already returns applied=true/false)
--   2. Achievement unlocks (trophy/medal/certificate earned at axis milestones)
--
-- Both flow through existing public.notifications table and are gated behind
-- platform_settings.notifications_enabled (default: false, per CLAUDE.md §9).
-- No new schema needed — only new behavior in the complete_task() function.
--
-- CHANGES:
-- 1. Augment public.complete_task() to include achievement-unlock logic:
--    - Compute milestones crossed (existing milestones_for_axis formula)
--    - Award certificates/trophies/medals for each new milestone
--    - Return unlocked array to client (already in place)
-- 2. On applied=true, insert notification rows if platform_settings.notifications_enabled
--    - Task completion message: "Completed: {p_task_name}"
--    - Achievement message: "Trophy Unlocked: Mastery {trophy_num}" | similar for medals/certs
--    - All use notification_type = 'task_complete'/'achievement'
--
-- DEPENDS ON:
--   - public.notifications table (omega_notifications_fix.sql / migrations/0091)
--   - public.complete_task() signature from omega_complete_task_dedup_fix.sql
--   - public.milestones_for_axis() from achievements.sql
--   - public.platform_settings table with notifications_enabled flag
--
-- Idempotent, safe to re-run. Uses CREATE OR REPLACE on complete_task().
-- ============================================================================
BEGIN;
...
COMMIT;
-- ===== end omega_task_and_achievement_notifications.sql =====
