...
| `omega-*.js` modules | 142 (1324 KB) |
| root `.js` files | 150 |
| `supabase/*.sql` (flat bag) | 127 |
| `supabase/migrations/*.sql` | 196 (106 numbered `NNNN_`, 90 timestamped) |
| Edge Functions | 14 |
| skills | 23 |
| agent definitions | 2 |
...
end-to-end for the first time"*. The 102 files added since (numbered and
timestamped alike) were **not part of that validation**, and no run has covered
all 196. Treat the validated scope as `0001`–`0094` only.
