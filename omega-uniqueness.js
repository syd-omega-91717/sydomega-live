/* SYD OMEGA 91717 — Content Uniqueness System v1.0
   ...
   Non-destructive. Drop anywhere. */
(function(){
  'use strict';
  if(window.__omegaUniqueness)return;
  window.__omegaUniqueness=true;

  const SEEN={titles:new Set(),purposes:new Set(),subjects:new Set()};
  const VIOLATIONS=[];
  ...
  function checkPage(){
    ...
    const issues=[];
    if(title&&SEEN.titles.has(title)) issues.push({type:'TITLE',value:title,msg:'Title already used by another page'});
    else if(title) SEEN.titles.add(title);

    if(purpose&&SEEN.purposes.has(purpose)) issues.push({type:'PURPOSE',value:purpose,msg:'Meta description duplicated'});
    else if(purpose) SEEN.purposes.add(purpose);

    const bodyText=document.body?document.body.innerText:'';
    const firstSentence=bodyText.split('.')[0].trim();
    if(firstSentence.length>20&&SEEN.subjects.has(firstSentence)) issues.push({type:'SUBJECT',value:firstSentence.slice(0,60)+'…',msg:'Opening sentence duplicates another page'});
    else if(firstSentence.length>20) SEEN.subjects.add(firstSentence);

    if(issues.length){
      VIOLATIONS.push({page:key,issues});
      console.warn('[Ω UNIQUENESS]',key,issues);
    }
    document.dispatchEvent(new CustomEvent('omega:uniqueness-checked',{detail:{page:key,issues,passed:issues.length===0}}));
  ...
  function renderGuard(el){
    if(!el)return;
    var issues=[];
    try{ var stored=JSON.parse(sessionStorage.getItem('omega:uniqueness:log')||'[]'); issues=stored; }catch(e){}
    ...
    if(issues.length===0){
      ...
      issues.forEach(function(v){
        ...
        v.issues.forEach(function(i){ html+='<div style=font-family:var(--M);font-size:7px;color:var(--muted);padding-left:8px>→ '+i.type+': '+i.msg+'</div>'; });
        html+='</div>';
      });
    }
    el.innerHTML=html;
  ...
  window.OmegaUniqueness={
    ...
    log:function(){
      var log=[];
      try{log=JSON.parse(sessionStorage.getItem('omega:uniqueness:log')||'[]');}catch(e){}
