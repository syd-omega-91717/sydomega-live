...
(function(){
  ...
  function mount(){
    ...
    inject('/omega-mission-control.js','data-omega-mission-control');
    injectStyle('/omega-command-palette.css','data-omega-command-style');
    inject('/omega-command-palette.js','data-omega-command-palette');
    injectStyle('/omega-mission-control.css','data-omega-mission-control-style');
    ...
    root.innerHTML='<div class="omega-v2-signal" aria-hidden="true"></div><div class="omega-v2-label"><span>Ω</span> SYSTEM ONLINE</div><div class="omega-v2-actions"><button class="omega-v2-mission" type="button" aria-label="Open Omega Mission Control">MISSION <kbd>M</kbd></button><button class="omega-v2-command" type="button" aria-label="Open Omega command interface">COMMAND <kbd>/</kbd></button></div>';
    document.body.appendChild(root);
    var btn=root.querySelector('.omega-v2-command');var mission=root.querySelector('.omega-v2-mission');
    ...
    btn.addEventListener('click',requestCommand);mission.addEventListener('click',function(){document.dispatchEvent(new CustomEvent('omega:mission_control'));});
    document.addEventListener('keydown',function(e){if(/input|textarea|select/i.test(document.activeElement.tagName))return;if(e.key==='/'||(e.metaKey&&e.key.toLowerCase()==='k')){e.preventDefault();btn.click();}else if(e.key.toLowerCase()==='m'){e.preventDefault();mission.click();}});
