...
| `supabase/migrations/*.sql` | 225 (106 numbered `NNNN_`, 119 timestamped) |
| Edge Functions | 14 |
| skills | 23 |
| agent definitions | 2 |
...
end-to-end for the first time"*. The 131 files added since (numbered and
timestamped alike) were **not part of that validation**, and no run has covered
all 225. Treat the validated scope as `0001`–`0094` only.
