-- SYD OMEGA 91717 — atomic Stripe webhook event boundary.
-- Additive correction. The existing Edge Function and public
-- apply_subscription() -> private.apply_subscription() boundary remain intact.
-- This adds one service-only RPC around that boundary so event idempotency and
-- entitlement mutation share one database transaction.
...
REVOKE ALL ON FUNCTION public.apply_subscription_event(uuid, text, text, text, timestamptz, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.apply_subscription_event(uuid, text, text, text, timestamptz, text, text) TO service_role;
