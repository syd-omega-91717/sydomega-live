...
(function(){
  ...
  function updateBadge(n){
    ...
    if(!badge){
      ...
      badge.style.cssText='position:fixed;top:16px;right:16px;z-index:500;background:var(--crim,#8B0000);color:#fff;font-family:var(--M,\'Courier Prime\',monospace);font-size:8px;font-weight:bold;width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 2px 8px rgba(139,0,0,.5)';
  ...
  function showToast(msg,type){
    ...
    t.innerHTML='<div style="font-family:var(--M,\'Courier Prime\',monospace);font-size:8px;letter-spacing:2px;color:'+col+';margin-bottom:4px">\u03A9 NOTIFICATION</div>'
      +'<div style="font-size:11px;color:#e9e6dc;line-height:1.5">'+msg+'</div>';
  ...
  function buildPanel(notifs){
    ...
    p.innerHTML='<div style="padding:12px 16px;border-bottom:1px solid rgba(201,168,76,.12);display:flex;align-items:center;justify-content:space-between">'
      +'<span style="font-family:var(--D,\'Cinzel Decorative\',serif);font-size:11px;color:var(--gold,#C9A84C)">\u03A9 NOTIFICATIONS</span>'
      +'<button onclick="document.getElementById(\'omega-notif-panel\').remove()" style="font-family:var(--M,\'Courier Prime\',monospace);font-size:9px;color:rgba(138,134,118,.5);background:none;border:none;cursor:pointer" aria-label="Close notifications">\u00d7 CLOSE</button>'
    +'</div>'
    +'<div style="max-height:50vh;overflow-y:auto">'
    +(!notifs.length?'<div style="padding:20px;text-align:center;font-family:var(--M,\'Courier Prime\',monospace);font-size:9px;color:rgba(138,134,118,.4)">NO NOTIFICATIONS</div>'
    ...
    +'<div style="padding:10px 16px;border-top:1px solid rgba(201,168,76,.08);text-align:center">'
      +'<a href="/notifications.html" style="font-family:var(--M,\'Courier Prime\',monospace);font-size:8px;letter-spacing:2px;color:var(--cyan,#00E5FF);text-decoration:none">VIEW ALL NOTIFICATIONS \u2192</a>'
    +'</div>';
    return p;
  }
