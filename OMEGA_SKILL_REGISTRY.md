...
| `omega-*.js` modules | 150 (1186 KB) |
| root `.js` files | 159 |
| `supabase/*.sql` (flat bag) | 126 |
| `supabase/migrations/*.sql` | 170 (106 numbered `NNNN_`, 64 timestamped) |
| Edge Functions | 11 |
| skills | 19 |
| agent definitions | 1 |
...
end-to-end for the first time"*. The 76 files added since (numbered and
timestamped alike) were **not part of that validation**, and no run has covered
all 170. Treat the validated scope as `0001`–`0094` only.
