...
| `notifications` table never populated | Table existed (once applied) but nothing inserted a row | `supabase/omega_notify_triggers.sql`, `migrations/0092` (5 RPCs now insert on event) | **Attempted, fixed, not yet re-applied** — first run against the live DB hit `42P13` (existing `grant_permanent_access` had a different return type); fixed by adding the same drop-prior-versions block `omega_access_control.sql` already uses for this exact scenario (`REPOSITORY_AUDIT.md` §6.16). Corrected file not yet re-run. |
