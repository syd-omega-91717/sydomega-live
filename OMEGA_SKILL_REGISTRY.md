...
| `omega-*.js` modules | 90 (807 KB) |
| root `.js` files | 98 |
| `supabase/*.sql` (flat bag) | 118 |
| `supabase/migrations/*.sql` | 117 (94 numbered `NNNN_`, 23 timestamped) |
| Edge Functions | 11 |
| skills | 8 |
| agent definitions | 1 |

`supabase/migrations/README.md` records that the **94-file
numbered sequence** (`0001`–`0094`) applies cleanly against a fresh scratch
PostgreSQL 16 instance. The 23 timestamped files added since were **not part of
that validation**, and no run has covered all
117. Treat the validated scope as the numbered sequence only.
