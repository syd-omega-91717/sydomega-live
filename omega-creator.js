...
return '<div style=margin-bottom:12px><label style=display:block;font-family:var(--M);font-size:12px;letter-spacing:2px;color:var(--muted);margin-bottom:4px;text-transform:uppercase>'+f+'</label><input type=text class=oc-input data-field='+f+' placeholder=Enter '+f+'… style=width:100%;background:rgba(0,0,0,.3);border:1px solid rgba(201,168,76,.15);border-radius:2px;padding:8px 10px;color:rgba(220,210,180,.8);font-family:var(--M);font-size:12px;outline:none></div>';
}).join('');
var typeOptions = IDEA_TYPES.map(function(t) {
return '<option value='+t+(t===type?' selected':'')+'>'+t.toUpperCase()+'</option>';
}).join('');
el.innerHTML = '<div style=font-family:var(--M);font-size:12px;letter-spacing:2px;color:var(--muted);margin-bottom:12px>OMEGA CREATOR · IDEA FORGE</div>' + '<div style=margin-bottom:12px><label style=display:block;font-family:var(--M);font-size:12px;letter-spacing:2px;color:var(--muted);margin-bottom:4px>TYPE</label><select class=oc-type style=width:100%;background:rgba(0,0,0,.3);border:1px solid rgba(201,168,76,.15);border-radius:2px;padding:8px 10px;color:rgba(220,210,180,.8);font-family:var(--M);font-size:12px;outline:none>'+typeOptions+'</select></div>' + fieldsHtml + '<button class=oc-submit style=font-family:var(--M);font-size:12px;letter-spacing:2px;padding:8px 16px;background:none;border:1px solid rgba(201,168,76,.3);color:var(--gold);border-radius:2px;cursor:pointer;transition:.12s>FORGE IDEA</button>';
...
html += '<div style=min-width:160px><div style=font-family:var(--M);font-size:12px;letter-spacing:2px;color:'+stage.color+';margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid rgba(201,168,76,.1)>'+stage.label+' <span style=color:var(--muted)>'+stageIdeas.length+'</span></div>';
stageIdeas.forEach(function(idea) {
html += '<div class=oc-card data-id='+idea.id+' style=background:rgba(201,168,76,.04);border:1px solid rgba(201,168,76,.1);border-radius:3px;padding:10px;margin-bottom:8px;cursor:pointer;transition:.12s>' + '<div style=font-family:var(--M);font-size:12px;color:var(--ink);margin-bottom:4px>'+(idea.data.title || idea.data.concept || idea.data.product || idea.data.feature || 'Untitled')+'</div>' + '<div style=font-family:var(--M);font-size:12px;color:var(--muted);text-transform:uppercase>'+idea.type+' · '+idea.votes+' votes</div>' + '</div>';
});
html += '</div>';
});
html += '</div>';
...
return '<div style=margin-bottom:8px><span style=font-family:var(--M);font-size:12px;letter-spacing:1px;color:var(--muted);text-transform:uppercase>'+k+':</span> <span style=font-family:var(--M);font-size:12px;color:var(--ink)>'+v+'</span></div>';
}).join('');
var commentsHtml = idea.comments.map(function(c) {
return '<div style=padding:6px 0;border-bottom:1px solid rgba(201,168,76,.06)><div style=font-family:var(--M);font-size:12px;color:var(--muted)>'+new Date(c.time).toLocaleDateString()+'</div><div style=font-family:var(--M);font-size:12px;color:var(--ink)>'+c.text+'</div></div>';
}).join('');
el.innerHTML = '<div class=glass style=padding:16px>' + '<div style=display:flex;justify-content:space-between;align-items:center;margin-bottom:12px><div style=font-family:var(--M);font-size:12px;letter-spacing:2px;color:'+(stageInfo?stageInfo.color:'var(--gold)')+'>'+(stageInfo?stageInfo.label:idea.stage)+'</div><button class=oc-advance style=font-family:var(--M);font-size:12px;letter-spacing:1px;padding:4px 10px;background:none;border:1px solid rgba(201,168,76,.2);color:var(--gold);border-radius:2px;cursor:pointer>'+(nextStage?'ADVANCE TO '+nextStage.label:'COMPLETE')+'</button></div>' + '<div style=font-family:var(--D);font-size:clamp(16px,2vw,22px);color:var(--gold);margin-bottom:12px>'+(idea.data.title || idea.data.concept || idea.data.product || idea.data.feature || 'Untitled Idea')+'</div>' + fieldsHtml + '<div style=margin-top:12px;padding-top:12px;border-top:1px solid rgba(201,168,76,.1)><div style=font-family:var(--M);font-size:12px;letter-spacing:2px;color:var(--muted);margin-bottom:8px>VOTES: '+idea.votes+'</div><button class=oc-vote style=font-family:var(--M);font-size:12px;letter-spacing:1px;padding:4px 10px;background:none;border:1px solid rgba(63,178,127,.3);color:var(--green);border-radius:2px;cursor:pointer>+1 VOTE</button></div>' + '<div style=margin-top:12px;padding-top:12px;border-top:1px solid rgba(201,168,76,.1)><div style=font-family:var(--M);font-size:12px;letter-spacing:2px;color:var(--muted);margin-bottom:8px>COMMENTS</div>'+(commentsHtml||'<div style=font-family:var(--M);font-size:12px;color:var(--muted)>No comments yet</div>')+'</div>' + '<div style=margin-top:12px><input type=text class=oc-comment-input placeholder=Add comment… style=width:100%;background:rgba(0,0,0,.3);border:1px solid rgba(201,168,76,.15);border-radius:2px;padding:6px 10px;color:rgba(220,210,180,.8);font-family:var(--M);font-size:12px;outline:none><button class=oc-comment-btn style=margin-top:6px;font-family:var(--M);font-size:12px;letter-spacing:1px;padding:4px 10px;background:none;border:1px solid rgba(201,168,76,.2);color:var(--gold);border-radius:2px;cursor:pointer>COMMENT</button></div>' + '</div>';
...
return '<div style=display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid rgba(201,168,76,.06)><span style=font-family:var(--M);font-size:12px;color:'+st.color+'>'+st.label+'</span><span style=font-family:var(--M);font-size:12px;color:var(--ink)>'+count+'</span></div>';
}).join('');
var typeHtml = IDEA_TYPES.map(function(t) {
var count = s.byType[t] || 0;
return '<div style=display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid rgba(201,168,76,.06)><span style=font-family:var(--M);font-size:12px;color:var(--muted);text-transform:uppercase>'+t+'</span><span style=font-family:var(--M);font-size:12px;color:var(--ink)>'+count+'</span></div>';
}).join('');
el.innerHTML = '<div style=display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px>' + '<div class=glass style=padding:16px><div style=font-family:var(--M);font-size:12px;letter-spacing:2px;color:var(--muted);margin-bottom:10px>TOTAL IDEAS</div><div style=font-family:var(--D);font-size:clamp(24px,4vw,36px);color:var(--gold)>'+s.total+'</div></div>' + '<div class=glass style=padding:16px><div style=font-family:var(--M);font-size:12px;letter-spacing:2px;color:var(--muted);margin-bottom:10px>PIPELINE STAGES</div>'+stageHtml+'</div>' + '<div class=glass style=padding:16px><div style=font-family:var(--M);font-size:12px;letter-spacing:2px;color:var(--muted);margin-bottom:10px>BY TYPE</div>'+typeHtml+'</div>' + '</div>';
}
};
