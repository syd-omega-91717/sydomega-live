...
| "Text too small" | **Done, and gated.** `scripts/type-scale.py` lifted every declaration below 12px to 12px across five surfaces — page `<style>` blocks, quoted *and* unquoted `style=` attributes, page `<script>` blocks, and root `.js` (the per-surface counts are in `FIXES_LOG.md`), and `--check` runs blocking in the contract suite so it cannot regress page-by-page again. The floor is a single value deliberately: a per-size map was tried first and was both non-idempotent and hierarchy-inverting (`FIXES_LOG.md`). What is still open is a real `--fs-*` token set, and the residual ~4% of rendered text that falls below 12px through `em`/`%` chains compounding down a nesting tree — that needs per-rule judgement, not a sweep. |
