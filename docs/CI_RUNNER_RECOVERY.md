...
```powershell
cd C:\actions-runner
.\config.cmd --unattended --replace --url https://github.com/syd-omega-91717/sydomega-live --token PASTE_TOKEN_HERE --name SYD-OMEGA-WIN --labels self-hosted,Windows,X64,syd-omega --work _work
.\svc.cmd install
.\svc.cmd start
...
**`svc.cmd` is generated by `config.cmd` — it is not in the download.** Running
`.\svc.cmd install` before a successful registration gives
`CommandNotFoundException`, which reads like a missing file but is a missing
step.

### 3. If the service was skipped because the shell was not elevated

Registration is already saved; only re-do the service part, elevated:
