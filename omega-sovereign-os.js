...
(function(){
  ...
  function mountVisualLayer(){
    ...
    function rail(){
      ...
      var el=document.createElement('div');el.className='omega-visual-rail';el.setAttribute('aria-hidden','true');
      (document.body||document.documentElement).appendChild(el);
    }
    if(document.body)rail();else document.addEventListener('DOMContentLoaded',rail,{once:true});
    EventBus.emit('visual_layer_ready',{version:'2.0.0',page:_page});
  }

  function mountCinematicEngine(){
    if(document.querySelector('script[data-omega-cinematic]'))return;
    var s=document.createElement('script');s.src='/omega-cinematic-engine.js';s.defer=true;s.setAttribute('data-omega-cinematic','1');
    (document.body||document.documentElement).appendChild(s);
    EventBus.emit('cinematic_engine_ready',{version:'1.0.0',page:_page});
  }

  mountVisualLayer();
  mountCinematicEngine();

  window.OmegaOS={version:OS_VERSION,session:SESSION_ID,events:EventBus,health:Health,circuit:CircuitBreaker,safeQuery:safeQuery,page:_page,PHI:PHI,EU:EU,APEX:27.8367,TRIAL:557,DEDICATION:33437,FORMULA:'sqrt(A³+B³+C³)×φ/e',visual:{version:'2.0.0',stylesheet:'/omega-visual-evolution.css',cinematic:'/omega-cinematic-engine.js'},auth:function(a,b,c){return Math.sqrt(Math.pow(a,3)+Math.pow(b,3)+Math.pow(c,3))*PHI/EU;}};
})();
