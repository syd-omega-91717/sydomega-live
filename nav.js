...
(function(){
  ...
  (function(){
    ...
    /* The rail goes INSIDE the content column, at its top. It used to be
       inserted before <main> -- i.e. into .shell, a flex ROW -- or as the
       first child of a flex-row <body>, so it rendered as a 315px-wide,
       full-height empty column beside the sidebar on every page (measured
       dashboard.html: x=80 w=315 h=3485), pushing the content right. A
       candidate that contains the sidebar is the shell itself, not the
       content column, and is skipped (CLAUDE.md section 4). */
    var side=document.getElementById('omega-side');
    var cands=document.querySelectorAll('main, .main, [role="main"], .page-shell, #app');
    var col=null;
    for(var ci=0;ci<cands.length;ci++){if(!(side&&cands[ci].contains(side))){col=cands[ci];break;}}
    if(col)col.insertBefore(rail,col.firstChild);
    else if(getComputedStyle(document.body).flexDirection!=='row'||getComputedStyle(document.body).display.indexOf('flex')<0)document.body.insertBefore(rail,document.body.firstChild);
