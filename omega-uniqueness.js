...
/* ============================================================================ SYD OMEGA 91717 — Content Uniqueness System v1.0 Prevents duplicate pages, titles, purposes, and explanations. Non-destructive. Drop anywhere: <script src=omega-uniqueness.js></script> ============================================================================ */
 (function () {
'use strict';
if (window.__omegaUniqueness) return;
window.__omegaUniqueness = true;
const SEEN = {
titles: new Set(), purposes: new Set(), subjects: new Set()
};
const VIOLATIONS = [];
function checkPage() {
...
const key = (location.pathname.split('/').pop() || 'index').replace(new RegExp("\\.html?$","i"), '');
const issues = [];
if (title && SEEN.titles.has(title)) issues.push({
type: 'TITLE', value: title, msg: 'Title already used by another page'
});
else if (title) SEEN.titles.add(title);
if (purpose && SEEN.purposes.has(purpose)) issues.push({
type: 'PURPOSE', value: purpose, msg: 'Meta description duplicated'
});
else if (purpose) SEEN.purposes.add(purpose);
const bodyText = document.body ? document.body.innerText : '';
const firstSentence = bodyText.split('.')[0].trim();
if (firstSentence.length > 20 && SEEN.subjects.has(firstSentence)) issues.push({
type: 'SUBJECT', value: firstSentence.slice(0, 60) + '…', msg: 'Opening sentence duplicates another page'
});
...
detail: {
page: key, issues, passed: issues.length === 0
}
}));
...
check: checkPage, violations: function () {
return VIOLATIONS;
}, renderGuard: renderGuard, log: function () {
var log = [];
try {
