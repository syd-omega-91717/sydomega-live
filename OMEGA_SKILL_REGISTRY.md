...
| `omega-*.js` modules | 149 (1397 KB) |
| root `.js` files | 157 |
| `supabase/*.sql` (flat bag) | 127 |
| `supabase/migrations/*.sql` | 219 (106 numbered `NNNN_`, 113 timestamped) |
| Edge Functions | 14 |
| skills | 23 |
| agent definitions | 2 |
...
end-to-end for the first time"*. The 125 files added since (numbered and
timestamped alike) were **not part of that validation**, and no run has covered
all 219. Treat the validated scope as `0001`–`0094` only.
