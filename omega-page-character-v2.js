...
(function () {
  ...
  function classify(key) {
    var map = {
      command: /command|dashboard|home/,
      identity: /identity|profile|passport|character|account|membership/,
      ...
      gaming: /gaming|game|arena|trophy|leaderboard/,
      media: /media|cinema|film|series|trailers|music|video/,
      commerce: /marketplace|commerce|merchant|store|shop|services/,
      creation: /forge|creator|creation|publishing|contribution|lab|project/,
      community: /social|family|contacts|feed|community|factions/,
      governance: /governance|govern|compliance|approvals|charter|covenant|terms/,
  ...
  function mount() {
    ...
    document.documentElement.style.setProperty('--omega-character-color',
      c.emblem === 'Ω' ? 'var(--gold)' : c.emblem === '◉' ? 'var(--cyan)' : c.emblem === '▶' ? 'var(--crim)' : 'var(--gold)');
    ...
    document.dispatchEvent(new CustomEvent('omega:page-character-v2', {
      detail: { page: key, character: type, manifest: c, levels: MOTION_LEVELS }
    }));
  }
  ...
  window.OmegaPageCharacterV2 = {
    ...
    get: function (key) {
      var type = classify(key);
      return { type: type, manifest: FRAMEWORK[type] };
    }
  };
})();
