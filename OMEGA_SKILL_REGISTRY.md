...
| `omega-*.js` modules | 150 (1183 KB) |
| root `.js` files | 159 |
| `supabase/*.sql` (flat bag) | 126 |
| `supabase/migrations/*.sql` | 168 (106 numbered `NNNN_`, 62 timestamped) |
| Edge Functions | 11 |
| skills | 19 |
| agent definitions | 1 |
...
end-to-end for the first time"*. The 74 files added since (numbered and
timestamped alike) were **not part of that validation**, and no run has covered
all 168. Treat the validated scope as `0001`–`0094` only.
